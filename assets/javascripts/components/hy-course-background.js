class HyCourseBackground extends HTMLElement {
  ...
  createItem (count) {
    ...
    const itemDistribution = ['ball', 'line']
    ...
    if (type === 'ball') {
      ...
      item.css('margin-left', `${Math.ceil(Math.random() * 20) * Math.ceil(Math.random() * 5) + 20}vw`)
    } else {
      item.css('width', `${Math.ceil(Math.random() * 3) * 0.3}rem`)
      item.css('height', `${Math.ceil(Math.random() * 20) + 20}rem`)
      item.css('margin-left', `${Math.ceil(Math.random() * 10) * Math.ceil(Math.random() * 5) + 50}vw`)
    }
